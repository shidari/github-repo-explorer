...
- session storage の代用として **Jotai** を採用（session storage は hydration の問題が発生したため切り替え。要検証）
...
- **設計**
  ...
  - UX を考慮した設計判断
- **コードレビュー**
  - AI が書いたコードは基本的に目を通し、意図と実装が合っているかを確認（CSS や汎用 UI はステートを持たず（一部例外あり）、外部とやり取りを行わないため、かつ最悪全面書き換えが可能なためレビューを軽めにした）
