...
- ページネーション（前後1ページ + 先頭・末尾 + 省略記号のスマート表示）
...
- **Suspense を活用した debounce で UX を維持しながら API リクエスト回数を抑制**（`UnsafeSingletonDebounce`）
...
- **検索結果とページネーションの Suspense 境界を分離し、不要な fallback（ページ移動時に両者が suspend する）をなくして UX を改善**
- **Jotai atom + `data-repo` 属性 + `scrollIntoView` でスクロール復元を自前実装**
...
- **RSC で直接データ取得 + `Link prefetch={true}` + RSC キャッシュ（10分）で詳細ページへの遷移を高速化**
...
- **OGP 対応のため動的メタデータ（`generateMetadata`）を生成**
- **エラー・Not Found 時に専用ページを表示し、ユーザーが行き止まりにならないよう配慮**（`error.tsx`, `global-error.tsx`, `not-found.tsx`）
...
- 2段構えの Rate Limit（Edge proxy + Hono ミドルウェア）

#### 工夫した点・こだわりポイント

- **依存の切り替え・型付きエラーハンドリングのために Effect TS を採用**
  - 結果・エラー・依存を型で表現し、エラーをパターンマッチできるためハンドリングの漏れを防止
- **クライアントから叩かれるものだけを Hono の API として実装し、RSC は `repository/query` を直接呼び出すことで不要なネットワークラウンドトリップを排除**
- **API の検証をフロントエンド経由で行うのは非効率と判断し、Swagger UI で手動検証**
- **外部サービスを使用する Web アプリの負荷対策として、2段構えの Rate Limit を実装**
  ...
  - 通常利用では debounce + SWR キャッシュにより 10req/min 程度に収まると想定。バーストリクエストは1段目で遮断
...
| **任せたいこと** | IO に依存しないインメモリな状態管理 | Suspense 対応のキャッシュ・再取得管理 |
...
- **SWR**（サーバーステート）
  ...
  - Suspense はレンダリングのサスペンドにより不要なレンダリングをなくし、表示を最適化するために使用
...
- **Storybook** でコンポーネントの検証だけを行えるようにした
...
- **session storage → Jotai**: hydration 問題を回避するためにインメモリ state に切り替え
- **URL state（searchParams）不採用**: searchParams 変更で Page 自体が再レンダリングされるため、自動検索 UI と相性が悪い
- **Suspense + Result 型によるエラーハンドリング分離**: API エラーは fetcher が Result 型で返しコンポーネント内でハンドリング。Error Boundary はネットワーク断のフォールバックに限定
- **shadcn/ui ベースだが Radix UI / Tailwind CSS を外し CSS Modules で直接実装**: 今回の要件では不要な依存と判断
- **Storybook カタログ先行開発**: カタログでデザインと動きを検証してからページに組み込むプロセス
...
- **Repository 層**: GitHub API への接続とドメインモデルへの変換
  - 操作単位にクラスとしてカプセル化（AWS SDK v3 の Command パターンに着想）
...
- **1段目: Edge proxy**
  ...
  - **IP rate limit（インメモリ）**: IP アドレス単位で 30req/min
...
- **Command パターンによる Repository カプセル化**: AWS SDK v3 に着想し、操作単位にクラス化。Repository パターンのインターフェース肥大化を回避
- **エラーの reason マッピング**: HTTP ステータスを `rateLimit` / `validation` / `serviceUnavailable` / `unknown` にマッピングし、上位層でのハンドリングを容易化
- **開発時もモックデータ**: GitHub API を極力叩かず rate limit を消費しない安全な開発フロー
...
- **プロパティベーステスト**: スキーマ変換は性質として記述しやすく、Star 数表示（1000 → 1.0k）等の境界値検証にも活用
- **カタログ先行開発**: Storybook でデザインと動きを検証してからページに組み込むプロセスにより、手戻りを削減
- **モックデータのレイヤー流用**: レイヤー切り替えで E2E テストにもそのまま流用できる設計
- **Play function** によるインタラクションテスト含む（Storybook）
- **末端部分のレビュー軽量化**: コアでなく容易に差し替え可能な部分（CSS・汎用 UI 等）にはレビューコストをかけすぎない判断
