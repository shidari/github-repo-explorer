...
- **Jotai でセッション情報に近いものを管理し、複雑さを軽減**
...
- **SWR で Suspense 対応のキャッシュ・再取得管理を委譲**
...
- **hono-trpc でクライアントからの API 呼び出しも型安全に**
- **OpenAPI スキーマの自動生成と Swagger UI を提供（開発環境のみ）**: API の検証をフロントエンド経由で行うのは非効率と判断し、Swagger UI で手動検証
...
- **レイヤーの test / production 切り替えを `NODE_ENV` だけで簡単にできるようにした**
- **テスト時は GitHub API・外部 DB に依存せず動作**
  - シード固定のモックデータとインメモリ DB（PGlite）を使用
  - Rate limit の閾値等もテスト用の値に差し替え可能
- **インターフェースを事前定義し、開発初期は部分的にモックを使いながら開発することで開発効率化に貢献**
  - GitHub API を極力叩かず rate limit を消費しない安全な開発フロー
...
- **極力厳密さ・堅牢さを保証するために多段階の複数の rate limiter を統合**
  - **1段目: Edge proxy**
